...
% open employees instances 
fid = fopen('dedication.cfg','r');
if fid<0, error('cannot open file %s\n',a); end;

% fill employee set with input data
while ~feof(fid)
    line = strtrim(fgetl(fid));
    if (isempty(line) || all(isspace(line)) || strncmp(line,'#',1) || strncmp(line,';',1)||strncmp(line,',',1));
        % no operation 
...
% G1                    Task Precedence Graph
% b                     The importance rate of swarm exp        1.1
% Nant                  Number of ants in a generation          400
% Ngen                  Generation number                       15
% arcnum                number of arc lines                     12
% employee              
% empnum                number of the employees                 10
% empsknum              number of the employees skills max      5
% task
% tasknum               number of the tasks                     10
% tasksknum             number of the task skills max           3
% wcost                 The weight of cost                      0.000001
% wdur                  The weight of duration                  0.1
% wover                 The weight of overwork                  0.1
% wpenal                Penalty constant                        100
% wreqsk                The weight of number of required skills 10
% wt_cost               The weight of task cost                 0.000001
% wt_dur                The weight of task duration             0.1
% wundt                 The weight of number of undone tasks    10
