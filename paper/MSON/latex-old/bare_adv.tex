...
Finally, we carry out a case study based on some well-known web service workflows and a real-world dataset (the D2D contact traces of MIT Reality dataset and the QoS data of QWS dataset). The comparison implies that our proposed approach outperforms traditional approaches, especially those considering stable and fully available mobille services.
...
%
%\textit{Definition 2 (MSON participant):} A MSON participant is a mobile service user who can be a service %provider or a requester. It is denoted by a two-tuple $u = (P, C)$, where:
%
...
%
%\textit{Definition 3 (Mobile Service Composition Schedule):} A Mobile Service Composition Schedule $mscs = (T, E)$ is expressed as a directed acyclic graph (DAG), where $T = \{ t_1, t_2, ..., t_n \}$ is a set of tasks and $E$ is a set of directed edges. An edge $e_{ij}$ of the form $(t_i,t_j)$ indicates that a data dependency between $t_i$ and $t_j$ exists and $t_i/t_j$ are the parent/chile tasks respectively. A child task is executed after all its parent tasks are completed.
...
\caption{A sample schedule}
\label{A sample schedule}
\end{figure}

A Mobile Service Composition Schedule $mscs = (T, E)$ is expressed as a directed acyclic graph (DAG), where $T = \{ t_1, t_2, ..., t_n \}$ is a set of tasks and $E$ is a set of directed edges. An edge $e_{ij}$ of the form $(t_i,t_j)$ indicates that a data dependency between $t_i$ and $t_j$ exists and $t_i/t_j$ are the parent/child tasks respectively. A child task is executed after all its parent tasks are completed. A sample Schedule is illustrated in Fig. 1. The most commonly-used composition constructs (a.k.a. composition patterns) for service orchestration include: Sequence, Choice, Parallel and Loop. We denote them by symbols $\rightarrow$, \encircle{C}, \encircle{P} and \encircle{L}, respectively.

An MSON supports mobile service composition schedule through service providers. If a mobile user is MSON participant, he can decide a set of services of his mobile device to expose to other participants, $P = \{es_1, es_2, ..., es_m \}$. Also he can perceive services exposed by other users, $S = \{s_1^{(i)}, s_2^{(j)}, ..., s_n^{(k)} \}$, $s_n^{(k)}$ means there $k$ candidate services for task $t_i$. It is assumed that a service provider can provide services in parallel.

The execution order of a schedule can be expressed by assigning an index to each task. The index ranges from $1$ to $m$ and the $i$-th item indicates the order of executing $t_i$. The relationship between each task and its index can be described by a function $l : T \rightarrow N^+$ and encoded as a vector containing a permutation of $1$ to $m$. If $i$ occurs before $k$ in order, it does not necessarily indicate that the execution of $t_i$ is earlier than $t_k$. The starting time of tasks is decided by the completion time of their preceding tasks.

\begin{figure}[!t]
\centering
\includegraphics[width=3in]{./img/DAG3.pdf}
...
\begin{tabular}{cc}
\hline
\bfseries Pattern &  \bfseries Availability \\
\hline
...
s.t \ \ \ : \ \ \ & \tau \le A
\end{align}
Where $A$ is user's availability on availability. $\tau$ is the estimated availability to accomplish the $mscs$, it can be get by aggregation functions shown in Table 1.

The derivation of $\tau$ requires some efforts. $\tau$ can be calculated as the estimated ending time of the last task in the $mssc$:
\begin{align}
\tau = d_m
\end{align}
...
$b_i$ is decided by the estimated ending time of its immediately preceding tasks and the time required for data transfer. Let $\gamma_i$ denote the estimated time that all earlier tasks executed in the same provider to $t_i$ finished, we have:
\begin{align}
...
\end{align}
where $^{*}t_i$ denotes the immediately preceding tasks of $t_i$ , i.e., those which directly connect $t_i$ through edges in the $mscs$.
...
\begin{align}
b_1 = \delta + e_{entry, 1}
\end{align}
where $\delta$ is the time between receiving a service composition request and a schedule plan is generated.
...
For the service selection and composition problem, the vector $X = \{s_{(1,i)}, s_{(2,j)}. . . , s_{(n,k)}\}$ can describe a feasible solution as a service composition with $n$ tasks. 
...
% To verify the the capability of finding the optimal mobile service composition of our algorithms, we compare KH with the basic GA algorithm, basic PSO algorithm and a brute-force algorithm.

% \textbf{Algorithm 1:} Improved Krill-herd mobile service composition algorithm (KH) which described in Section 4.
...
% In this experiment, we compared the run time of KH with the other algorithms to evaluate the scalability of our algorithm.
...
% Among all the algorithms, PSO has the lowest run time, then KH, finally GA. But PSO's poor optimality makes it unsuitable for this problem, KH runs almost five times faster than GA. Based on the experimental results, we can conclude that KH maintains acceptable performance (optimality and scalability) with large data sets for both tasks and candidates.
