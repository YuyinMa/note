...
$\tau$        &   The estimated time required to accomplish a service composition \\
...
\STATE Estimate completion time of each krill individual in $X$
\FOR{$j=1$ \TO $MI$}
  \FOR{$k=1$ \TO $PS$}    
  ...
  \ENDFOR
\ENDFOR
\STATE Evaluate all individuals and return the best one
\end{algorithmic}
\end{algorithm}
...
The result of case study shows that the more complex composition template is, the better our algorithm perform.
