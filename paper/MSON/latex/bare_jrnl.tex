...
\IEEEPARstart{R}{ecent years} have witnessed the rapid development of mobile devices (e.g., smartphones, tablets, wearable devices, etc.) and mobile communication. Mobile devices are changing the way people getting the information and their daily lives. These devices they allow them to enjoy multiple ways of communicating almost anywhere at anytime \cite{satyanarayanan2010mobile}.
...
2) Neglect of deadline-constrain: Unlike large workflows executed in stationary IaaS cloud for hours, a mobile service composition usually completed within a few seconds to tens of seconds in highly dynamic mobile environment. Some studies \cite{nakhimovsky2009mobile, merz2016perceived, zhou2016define} have concluded that mobile users can tolerance a maximum of 38 seconds for a request in average. So if a service composition algorithm does not consider deadline-constrain, it is more likely to generate composition schedules with high makespan which brings bad user experience. Besides, these high-makespan schedules usually have low reliability, which will result in low success rate and high SLA (Service-level-agreement) violated rate. Thus, it is necessary to introduce deadline-constrained mechanism in a highly dynamic mobile environments.
...
As shown in Fig. 5, a mobile user can perceive services exposed by other users and these available services can be described as service pool $P = \{s_1^{(i)}, s_2^{(j)}, ..., s_m^{(k)} \}$, $s_m^{(k)}$ means there are $k$ candidate services for task $t_m$. Once a user issues a service composition request, tasks in the composite service are scheduled to the service selected from services pool and executed.
...
\tau = d_{exit}
\end{align}
where $d_{exit}$ denotes the estimated end time of task $t_{exit}$, the estimated end time of task $t_i$, $d_i$, can be iteratively calculated as:
\begin{align}
d_i = e_i + b_i
\end{align}
...
\subsection{Encoding}
A service composition schedule is encoded as a krill individual, and the individual with the best position stands for the so-far-obtained best schedule in terms of its corresponding estimated reliability of a composite service. The target of KH is to find an individual with the best position, which means to find the best mobile service composition with the best reliability. Therefore, once the optimal individual is found, the best mobile service composition is obtained.
...
Crossover operators aim at enhancing the search capability. Inspired by Stud Genetic algorithm (SGA) \cite{wang2014stud} that employs the optimal genome for crossover at each generation, we introduce a stud selection procedure to further improve KH's search capability.
...
\end{equation}
where $U_j$ and $L_j$ are upper and lower bounds of candidate services for the $j$-th task, respectively. $C_T$ is a constant value to scale the searching space, a lower value of $C_T$ lead a slower motion of krill, which let KH to search the searching space carefully.
...
Suppose the mobile user has submitted a composite service request with $m$ tasks, and there are $k$ available service provider within transmission range. $S$ and $G$ are two constant parameters indicate the population size and max iteration generations of the proposed KH-based algorithm
...
Finally, the total time complexity of KH-based composition algorithm is $O(km) + O(m+|E|) \approx O(km+|E|)$.
...
We employ representative existing algorithms \cite{Deng2017} \cite{sadiq2015service} which consider time-invariant availability and our proposed one to schedule mobile service compositions based on three well-known and 1 random generated composition templates given in Fig. 8. 
The 3 well-known templates represent composite services and business processes for travel-arranging, TensorFlow-based data processing, and astronomical image processing applications. As usually done in various existing studies \cite{Meena2016Cost, Rodriguez2014Deadline}, data transmission time can be calculated as the throughput (20Mbps as suggested by the MIT Reality dataset) divided by the average bandwidth. 
...
Fig. 9(b), Fig. 10(b), Fig. 11(b) and Fig. 12(b) shows the comparisons of process completion time where our proposed algorithm achieves 2.2\%, 17.7\%, 38.2\%, and 17.7\% time savings in each case over its peer. 
Intuitively, our algorithm outperforms traditional ones because traditional ones consider fully available mobile services and thus they tend to choose services with low response time, and thus fast services with high risks of unavailability due to high mobility are selected. 
