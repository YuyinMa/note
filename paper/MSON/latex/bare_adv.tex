...
​2) $P$ is the set of mobile services exposed to other MSON participant.

​3) $C$ is the set of discovered mobile services from nearby MSON participant.
...
​2) $S = \{ms_{(1,j)}, ms_{(2,k)},...,ms_{(n,l)}\}$ is a set of selected concrete services. For example, $ms_{1j}$ means the $j$-th service candidate is selected to execute $task_1$.
...
Note that most of wireless transaction protocol have defined the Received Signal Strength Indicator, then distance $d$ between mobile user $i$ and user $j$ can be calculated by signal strength \cite{Yang2010}, Finally let us discuss how to calculate $r$.
...
\end{equation}
Suppose a mobile service $ms$ running on mobile node $j$ is a candidate service for a task requested by user $i$, and the availability of candidate service $ms$ can be denoted as $q_{ava}(s)$, that is
\begin{flalign}
&\text{$q_{ava}(ms) = P_{i \ni j}$}&
\end{flalign}
\begin{align*}
...
% The whole process of the SKH algorithm is shown as fig.4.
...
Motion operator is the core of SKH algorithm. As shown in equation (10), the position of each krill individual is determined by three main factors: 1) motion influenced by other krill; 2) foraging action; and 3) physical diffusion. 
\begin{equation}
\frac{dX_i}{dt} =N_i+F_i+D_i
\end{equation}

where individual $X_i = \{ms_{(1,j)}, ms_{(2,k)}, . . . , ms_{(n,l)}\}$ represents the $i$-th composition service instance $CSI_i$, $n$ is the number of tasks in the service composition, $N_i$, $F_i$, and $D_i$ denote the motion influenced by other krill individuals, the foraging motion, and the physical diffusion, respectively.
...
$\alpha_i$ is the direction of the induced motion and it can be evaluated by target swarm density (target effect $\alpha^{target}$), local swarm density (local effect $\alpha^{local}$). $N^{max}$ is the maximum induced speed, $\omega_n \in [0, 1]$ the inertia weight of the induced motion, $N^{old}_{i}$ is the last induced motion influenced by other krill individuals.
...
The crossover operator plays an important role in Genetic algorithm for global optimization, we use this operator in SKH algorithm to enhance the search capability. The crossover operator in this paper is controlled by crossover rate $C_{rate}$ which can be obtain as follow:
\begin{equation}
C_{rate} = Cr + (1-Cr) \times \frac{U_{best}-U_{i}}{U_{best}-U_{worst}}
\end{equation}

Where $C_r$ is a pre-set fixed crossover rate, $U_{i}$ is the $i$-th individual's QoS Utility, $U_{best}$ is the best QoS Utility value so far, similarly, $U_{worst}$ is the worst QoS Utility value. 

Then we can use $C_{rate}$ to generate $i$-th individual's crossover vector $Cv = \{c_{1},c_{2},...,c_{n}\}$, it can be manipulated as:
...
Inspired by SGA \cite{khatib1998stud} (a type of GA that employs the optimal genome for crossover at each generation), we introduce stud selection procedure to improve SKH's search capability.

From algorithm 1, we can see that for each individual $X_i$ to crossover, we choose the optimal individual $Stud$ (i.e., individual with highest QoS Utility value) to mating. As shown in fig.4, the characteristics from $Stud$ are copied to $X$ according to crossover vector $Cv$. 
...
\STATE Sort all krill individuals in population $X$ by its fitness value, get optimal individual $Stud$, save best fitness value as $U_{best}$ and worst fitness value as $U_{worst}$
...
\ENDFOR
\FOR{$i=0$ \TO $taskNumber$}
\STATE $X_i[i] \leftarrow X_i \verb'&'  (1-Cv[i]) + Stud \verb'&' Cv[i]$ 
\ENDFOR
\end{algorithmic}
\end{algorithm}

\subsection{Update position}
After the crossover operator of each iteration, the offspring should be evaluated and updated to current evolutionary sequence.
...
where $d$ is the tasks number of composition service, $UB_j$ and $LB_j$ are upper and lower bounds of candidate services for the $j$-th task, respectively. $C_t$ is a constant value to scale the searching space. Finally, the overall SKH algorithm process can be describe in Algorithm 2.
...
\STATE Evaluate the fitness value of each krill individual in $X$
\FOR{$i=0$ \TO $MI$}
	\FOR{$i=0$ \TO $PS$}
		\STATE $X_i^{'} \leftarrow motionOperator()$
		\STATE $X_i^{''} \leftarrow crossoverOperator(X_i^{''})$
		\STATE $U_i^{'} \leftarrow evaluateFitness(X_i^{'})$
		\STATE $U_i^{''} \leftarrow evaluateFitness(X_i^{''})$
		\IF{$U_i^{''} < U_i^{'}$}
...
There are four parameters can be adjusted to improve the SKH's performance: population size $PS$, maximum iteration number $MI$, crossover rate $Cr$, foraging speed $V_f$, maximum induced speed $N_max$ and physical diffusion speed. As shown in Table 2, we generate six groups of parameters configuration to evaluate the impact of each parameter. For each group of parameters configuration, we tune one parameter and fix the other parameters. For each configuration setting, the SKH algorithm is executed 50 times independently and the average performance was recorded.
...
Figure 5 shows the results of tuning different parameters for SKH. 
...
Similarly, Figure 5(b) shows that the value of QoS Utility significantly increased for higher number of iteration times until to a limit: $MI = 60$. 
Figure 5(c) shows the impact of the crossover rate $Cr$. The performance of SKH increases with $Cr$ firstly, then decrease, the best performance is achieved for $Cr = 0.6$.
Figure 5(d) shows the impact of the foraging speed $V_f$. The performance of SKH increases with $V_f$ firstly, then decrease, the best performance is achieved for $V_f = 0.8$.
Figure 5(e) shows the impact of the induced speed $N_{max}$. The performance of SKH increases with $N_{max}$ firstly, then decrease, the best performance is achieved for $N_{max} = 0.3$.
Figure 5(F) shows the impact of the physical diffusion speed $D_{max}$. It shows that with the increase of $D_{max}$ the performance of SKH fluctuation slightly, we random generate $D_{max}$ from $[0.2 \sim 0.5]$ in this paper.
...
\textbf{Algorithm 1:} KH-based mobile service composition algorithm (SKH) which described in Section 4.
...
The result in Figure 6 shows that, as expected, the brute-force algorithm leads to the best performance with the highest fitness. The basic PSO algorithm has the worst performance with the lowest fitness values. Both SKH and Ga can find over 90\% optimal composition before 50 iterations and SKH performs little better than the basic GA algorithm. Although GA performs closely to SKH, we finally choose SKH because it runs almost five times faster than GA, in next subsection we will evaluate the scalability of above four algorithms.

\subsection{Scalability Evaluation}
In this experiment, we compared the run time of SKH with the other algorithms to evaluate the scalability of our algorithm.
...
In order to evaluate the impact of task number on the execution time of our algorithm, The task number is varied from 6 to 30. For each task number, the simulation is repeated 50 times, and the average value is recorded as the result. We observe from figure 7(b) that the run time of brute-force method is almost exponential to the problem size, while the run time of other three meta-heuristic algorithms (SKH, GA, PSO) are increases almost linearly with tasks increasing. In mobile environment, due to the dynamic movement and limited of computing resources, algorithm must meet the requirement of finding feasible composition within a short time. Therefore, although brute-force can obtain the optimal result, it is not suitable to the problem due to its poor scalability. 
...
Among all the algorithms, PSO has the lowest run time, then SKH and then GA, note that SKH runs almost five times faster than GA, but PSO's poor optimality makes it unsuitable for this problem. Based on the experimental results, we can conclude that SKH maintains acceptable performance (optimality and scalability) with large data sets for both tasks and candidates.
